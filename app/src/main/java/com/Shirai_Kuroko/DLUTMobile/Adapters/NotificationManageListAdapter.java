...
public class NotificationManageListAdapter extends RecyclerView.Adapter<NotificationManageListAdapter.ViewHolder> {
    private final NotificationManageActivity mContext;
    private List<NotificationHistoryDataBaseBean> mDatas;
    private List<NotificationHistoryDataBaseBean> Selected;

    public NotificationManageListAdapter(NotificationManageActivity context, List<NotificationHistoryDataBaseBean> datas, List<NotificationHistoryDataBaseBean> Slected) {
        mContext = context;
        mDatas = datas;
        Selected = Slected;
    }

    public void UpdateSelected(List<NotificationHistoryDataBaseBean> Slected)
    {
        Selected = Slected;
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int i) {
        ...
        View view = LayoutInflater.from(mContext).inflate(R.layout.item_notice_layout_manage,viewGroup,false);
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        holder.card_view_check_box.setOnCheckedChangeListener((compoundButton, b) -> {
            if (b)
            {
                mContext.AddSelected(payload);
            }
            else
            {
                mContext.RemoveSelected(payload);
            }
        });
            holder.card_view_check_box.setChecked(Selected.contains(payload));
    }
